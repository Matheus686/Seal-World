...
def login():
    ...
    if user:
        ...
        return redirect(url_for("index", erro=1))  # Redireciona com parâmetro de erro
...
def deletar(id):
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("UPDATE tarefas SET apagada = 1 WHERE id=? AND usuario_id=?", (id, session["usuario_id"]))
    conn.commit()
    conn.close()
    return redirect(url_for("listar_tarefas"))
