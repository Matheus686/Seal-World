...
#define MAX_DATA 11
#define USERNAME "admin"  // Nome de usuário fixo
#define PASSWORD "1234"  // Senha fixa
...
// Modificação na função adicionarTarefa
void adicionarTarefa(Tarefa** lista, Tarefa* nova) {
    // Se a nova tarefa for de alta prioridade, insere no início da lista
    if (strcmp(nova->prioridade, "Alta") == 0) {
        // Incrementa o ID das tarefas existentes
        Tarefa* temp = *lista;
        while (temp != NULL) {
            temp->id++;
            temp = temp->prox;
        ...
        *lista = nova;
        ...
        // Caso contrário, adiciona no final da lista
        if (*lista == NULL) {
            *lista = nova;
        } else {
            Tarefa* temp = *lista;
...
int main() {
    ...
    if (!login()) {
        return 0;  // Se falhar no login, sai do programa
    }
